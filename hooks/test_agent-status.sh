...
# 1) session_id из stdin payload, состояние working, pid из env
echo '{"session_id":"sess-1","cwd":"/x"}' | \
  AGENT_TRAFFIC_DIR="$TMP" AGENT_TRAFFIC_PID=99999 sh "$SCRIPT" working
F="$TMP/sess-1.json"
[ -f "$F" ] || fail "файл не создан"
jq -e '.state=="working" and .pid==99999 and .session_id=="sess-1" and (.ts|type=="number")' "$F" >/dev/null \
