...
- **PID reuse → возможный фантомный 🟢.** Упавшая в состоянии `working` сессия
  (закрыли вкладку, не прислав `Stop`/`SessionEnd`) остаётся как ⚠️ навсегда. macOS
  переиспользует PID'ы за ~сутки; если тот же PID займёт другой живой процесс, ⚠️
  может «ожить» в 🟢. Снимается кнопкой «Очистить ⚠️». Возможный фикс — TTL по полю
  `ts` (записывается, но пока не используется консьюмером).
